      ...
      <View style={tailwind.style('flex-1 bg-brand-background dark:bg-brand-background-dark')}>
        <ScrollView
          ...
          <View style={tailwind.style('pt-6 gap-4')}>
            <Animated.Text
              style={tailwind.style(
                'text-2xl text-gray-950 dark:text-grayDark-50 font-inter-semibold-20 text-center',
              )}>
              {i18n.t('MFA.TITLE')}
            </Animated.Text>
          </View>
          ...
          <View style={tailwind.style('flex-row mt-8 mb-6 bg-gray-100 dark:bg-grayDark-100 rounded-lg p-1')}>
            <Pressable
              style={tailwind.style(
                `flex-1 py-3 px-4 rounded-md ${activeTab === 'authenticator' ? 'bg-brand-background dark:bg-brand-background-dark' : ''}`,
              )}
              onPress={() => {
              ...
              <Text
                style={tailwind.style(
                  `text-center font-inter-normal-20 ${
                    activeTab === 'authenticator'
                      ? 'text-gray-950 dark:text-grayDark-50'
            ...
            <Pressable
              style={tailwind.style(
                `flex-1 py-3 px-4 rounded-md ${activeTab === 'backup' ? 'bg-brand-background dark:bg-brand-background-dark' : ''}`,
              )}
              onPress={() => {
                setActiveTab('backup');
                setIsCodeWrong(false);
            ...
            {activeTab === 'authenticator' ? (
              <>
                ...
                {error && (
                  <Animated.Text
                    style={tailwind.style(
                      'font-inter-normal-20 text-ruby-900 dark:text-rubyDark-900 mb-6 pl-2',
                    )}>
                    {error}
                  </Animated.Text>
                )}
              ...
              <>
                <Animated.View style={[rShakeStyle, tailwind.style('mb-2 pl-2 pr-2')]}>
                  <TextInput
                    ref={backupInputRef}
                    style={tailwind.style(
                      'w-full p-4 border-2 rounded-lg text-left border-gray-300 dark:border-grayDark-300 text-gray-950 dark:text-grayDark-50 bg-blackA-A4 dark:bg-grayDark-900',
                    )}
                    value={backupCode}
                    onChangeText={text => {
                      setBackupCode(text);
